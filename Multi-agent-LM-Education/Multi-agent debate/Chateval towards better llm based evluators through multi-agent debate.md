...
#### 1. One by one: 
During each round of debate, the ddebater agents take turns in a set of order to generate their responses on the current observation. When it's time for a debator agent to respond